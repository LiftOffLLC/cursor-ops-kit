...
function validateEnvironment() {
  ...
  try {
    ...
    warn('Git is not installed. Some features like git hooks will not be available.');
    process.env.SKIP_GIT = '1';
  }
  ...
  try {
    ...
    warn('Not a git repository. Git-related features will be skipped.');
    process.env.SKIP_GIT = '1';
  }
}
...
function copyAll() {
  try {
    ...
    // Only add git-related files if git is available
    ...
    if (!process.env.SKIP_GIT) {
      ...
      warn('\nGit-related files were skipped. Initialize git to enable all features.');
    }
    ...
    if (!process.env.SKIP_GIT) {
      ...
      info('  3. Initialize git to enable additional features');
      info('  4. Run cursor-ops init again after git is initialized');
    }
